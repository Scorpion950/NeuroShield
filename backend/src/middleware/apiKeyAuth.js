...
async function apiKeyAuth(req, res, next) {
  try {
    ...
    const keyRecord = await queryOne(`
      ...
      WHERE ak.key_value = $1 AND ak.is_active = 1
    `, [apiKey]);
    ...
    execute('UPDATE api_keys SET last_used = NOW() WHERE key_value = $1', [apiKey]).catch(() => {});

    req.apiKey = keyRecord;
    req.sourceApp = { id: keyRecord.app_id, name: keyRecord.app_name };
    next();
  } catch (err) {
